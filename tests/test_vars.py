...
def test_criteria_local():
    ...
    search = odg.Gateway(approach="region", kw=kw, criteria=my_custom_criteria,
                         var_def=my_var_def, variables='salt')
...
def test_criteria_nonlocal():
    ...
    search = odg.Gateway(approach="region", kw=kw, criteria=criteria,
                         var_def=var_def, variables='salt')
    assert 'sea_water_practical_salinity' in search.sources[0].variables
...
def test_criteria_gateway():
    ...
    data1 = odg.Gateway(
        approach="region", readers=odg.erddap, kw=kw,
        erddap={'known_server': 'ioos'},
        criteria=my_custom_criteria, var_def=my_var_def, variables='salt'
    )
    data2 = odg.Gateway(
        approach="region", readers=odg.erddap, kw=kw,
        erddap={'known_server': 'ioos'},
...
def test_criteria_axds():
    ...
    kwargs = {'criteria': my_custom_criteria,
              'kw': kw,
              'approach': 'region',
              'parallel': False,
              'variables': 'salt',
...
def test_remove_variable():
    ...
    search = odg.Gateway(approach="region", kw=kw, criteria=my_custom_criteria,
                         readers=odg.erddap, erddap=dict(known_server='ioos'),
                         var_def=my_var_def, variables=['ssh', 'salt'])
...
def test_var_def_criteria():
    ...
    # add another entry
    my_var_def['u'] = {"units": "m/s", "fail_span": [-10, 10],
                       "suspect_span": [-5, 5]}

    with pytest.raises(AssertionError):
        assert odg.Gateway(approach="region", kw=kw,
                           criteria=my_custom_criteria, var_def=my_var_def)
